...
import omtteam.openmodularturrets.client.gui.slot.FuelSlot;
import omtteam.openmodularturrets.client.gui.slot.RedstoneSlot;
...
public class PowerGeneratorContainer extends Container {
    private final PowerGenerator tileEntity;

    public PowerGeneratorContainer(InventoryPlayer inventoryPlayer, PowerGenerator te) {
        this.tileEntity = te;

        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 2; y++) {
                this.addSlotToContainer(new FuelSlot(te.getInventory(), y + x * 3, 62 + y * 18, 17 + x * 18));
            }
        }
        this.addSlotToContainer(new RedstoneSlot(te.getInventory(), 6, 71, 50)); // Redstone Slot
