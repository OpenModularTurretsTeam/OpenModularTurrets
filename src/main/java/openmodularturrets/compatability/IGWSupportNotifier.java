            ...
            event.player.addChatComponentMessage(
                    IChatComponent.Serializer.func_150699_a(
                            "[\"" + EnumChatFormatting.GOLD
                                    + "The mod "
                                    + supportingMod
                                    + " is supporting In-Game Wiki mod. "
                                    + EnumChatFormatting.GOLD
                                    + "However, In-Game Wiki isn't installed! "
                                    + "[\","
                                    + "{\"text\":\"Download Latest\",\"color\":\"green\",\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/igwmod_download\"}},"
                                    + "\"]\"]"));
    ...
    private class ThreadDownloadIGW extends Thread {
        ...
        public void run() {
            try {
                ...
                connection.connect();
                String fileName = "IGW-Mod.jar";// connection.getHeaderField("Content-Disposition").replace("attachment;
                                                // filename=", "").replace("\"", "");
