...
public abstract class TurretHead extends TileEntity {
    ...
    Entity getTargetWithMinRange() {
        return TurretHeadUtil.getTargetWithMinimumRange(
                base,
                worldObj,
                base.getyAxisDetect(),
                xCoord,
    ...
    Entity getTargetWithoutEffect() {
        return TurretHeadUtil.getTargetWithoutSlowEffect(
                base,
                worldObj,
                base.getyAxisDetect(),
                xCoord,
    ...
    private Entity getTarget() {
        return TurretHeadUtil.getTarget(
                base,
                worldObj,
                base.getyAxisDetect(),
                xCoord,
    ...
    boolean chebyshevDistance(Entity target, TurretBase base) {
        return MathHelper.abs_max(
                MathHelper.abs_max(target.posX - this.xCoord, target.posY - this.yCoord),
                target.posZ - this.zCoord) > (getTurretRange() + TurretHeadUtil.getRangeUpgrades(base));
    }
    ...
    public void updateEntity() {
        ...
        if (base == null || base.getBaseTier() < this.turretTier) {
            ...
            if (target != null && target instanceof EntityPlayerMP) {
                ...
                if (TurretHeadUtil.isTrustedPlayer(entity.getUniqueID(), base) || entity.capabilities.isCreativeMode
            ...
            if (this.requiresAmmo()) {
                if (this.requiresSpecificAmmo()) {
                    for (int i = 0; i <= TurretHeadUtil.getScattershotUpgrades(base); i++) {
                        ...
                        if (ammo == null) {
                            ammo = TurretHeadUtil
                                    .getSpecificItemFromInvExpanders(worldObj, new ItemStack(this.getAmmo()), base);
                        }
                    }
                } else {
                    for (int i = 0; i <= TurretHeadUtil.getScattershotUpgrades(base); i++) {
            ...
            for (int i = 0; i <= TurretHeadUtil.getScattershotUpgrades(base); i++) {
                ...
                double accuracy = this.getTurretAccuracy() * (1 - TurretHeadUtil.getAccuraccyUpgrades(base))
                        * (1 + TurretHeadUtil.getScattershotUpgrades(base));
                ...
                double dist2 = MathHelper
                        .sqrt_double(adjustedX * adjustedX + adjustedY * adjustedY + adjustedZ * adjustedZ);
                float speedFactor = (float) (dist2 / dist);

                if (projectile.gravity == 0.00F) {
                    projectile
                            .setThrowableHeading(adjustedX, adjustedY, adjustedZ, 3.0F * speedFactor, (float) accuracy);
                } else {
                    projectile
                            .setThrowableHeading(adjustedX, adjustedY, adjustedZ, 1.6F * speedFactor, (float) accuracy);
                }
